...
func (c *concaver) computeFromSorted(convexHull FlatPoints) (concaveHull FlatPoints) {
	...
	indices := xy.SimplifyFlatCoords([]float64(concaveHull), c.seglength, 2)
	concaveHull = newHull(concaveHull, indices)
	return concaveHull
}

func newHull(hull []float64, indices []int) []float64 {
	newhull := make([]float64, 2*len(indices))
	for i, index := range indices {
		newhull[i*2] = hull[index*2]
		newhull[i*2+1] = hull[index*2+1]
	}
	return newhull
}
